...
public class ProjectsController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<ProjectResponseDto>>> GetById(int id)
    {
        ...
        var project = await _projectService.GetProjectEntityAsync(id, includeUnapproved: isAdmin);
    ...
    public async Task<ActionResult<ApiResponse<IEnumerable<ProjectMemberResponseDto>>>> GetMembers(int projectId)
    {
        ...
        return Ok(ApiResponse<IEnumerable<ProjectMemberResponseDto>>.SuccessResponse(members));
    }
    /// <summary>
    ...
    /// <summary>
    /// Retrieves all unapproved (pending) projects. Admin only.
    /// </summary>
    /// <returns>List of pending projects.</returns>
    /// <response code="200">Returns the list of pending projects.</response>
    [HttpGet("pending")]
    [Authorize(Policy = "AdminOnly")]
    public async Task<ActionResult<ApiResponse<IEnumerable<ProjectResponseDto>>>> GetPending()
    {
    ...
    /// <summary>
    /// Approve a pending (unapproved) project. Admin only.
    /// </summary>
    /// <param name="id">The ID of the project to approve.</param>
    /// <returns>Returns the approved project.</returns>
    /// <response code="200">Project approved successfully.</response>
    /// <response code="404">Project not found.</response>
    [HttpPost("{id}/approve")]
    [Authorize(Policy = "AdminOnly")]
    public async Task<ActionResult<ApiResponse<ProjectResponseDto>>> Approve(int id)
    {
        var project = await _projectService.GetProjectEntityAsync(id, includeUnapproved: true);
        if (project is null)
            return NotFound($"Project with ID {id} not found");

        await _projectService.ApproveAsync(id);

        return Ok(ApiResponse<ProjectResponseDto>.SuccessResponse(
